...
export default {
    ...
    collections: {
        ...
        archivedPostsByDateDescending: function (collectionsApi) {
            return collectionsApi
                .getFilteredByGlob("content/archive/*.md")
                .sort(function (a, b) {
            ...
            if (isAnIcon) {
                ...
                if (!isNaN(fontSizeParse)) {
                    if (fontSizeParse < 11 && fontSizeParse > 0) {
                        ...
                        console.log(
                            `icon shortcode "${iconName}" is a valid icon but "${fontSizeParse}" is not a valid icon size, icon size must be an integer from 1 to 10`
                        );
                    }
                }
                ...
                console.log(
                    `icon shortcode "${iconName}" is not a valid icon, see https://eleventy-libdoc.netlify.app/creating-content/widgets/icons/`
                );
            }
            return markup;
        },
        iconCard: async function (mainText, description, iconName) {
            ...
            if (typeof mainText == "string" && typeof description == "string") {
                ...
                if (!isAnIcon && iconName !== undefined) {
                    console.log(
                        `iconCard shortcode: ${iconName} is not a valid icon, default ${finalIconName} applied.`
                    );
                }
                markup = `
